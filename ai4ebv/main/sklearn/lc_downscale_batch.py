"""Train a scikit-learn classifier for all tiles."""
...
from ai4ebv.main.io import (ROOT, TS_PATH, CLASS_PATH, TRAIN_PATH, LC_LAYERS,
                            DEM_LAYERS)
...
if __name__ == '__main__':
    ...
    state = HLSDataset.state_file(
        ...
        npixel=NPIXEL, features=True, dem=DEM, qa=APPLY_QA, months=MONTHS,
        mode='batch', use_indices=USE_INDICES, year=YEAR, seasonal=SEASONAL)
    ...
    # -------------------------------------------------------------------------
    # AGGREGATE COLLECTED TRAINING DATA OVER ALL TILES ------------------------
    # -------------------------------------------------------------------------

    # convert training data to numpy arrays
    inputs = np.asarray(train_data['inputs'])
    ...
    for tile in TILES:
        ...
        y_pred, y_prob = predict_hls_tile(
            hls_ts, clf, tile_size=TILE_SIZE, use_indices=USE_INDICES,
            features=True, dem=dem)
